...
var layer1icon = L.OpenFreightMarkers.icon({
	...
	layer3icon = L.OpenFreightMarkers.icon({
        icon: 'community', markerColor: 'gold', layer:'layer3', title: 'Areawide Projects'}, IconPresets),
...
var layer3 = L.geoJson(null, {
    style: {fillColor: "#f0efa8", fillOpacity:.50, weight:3, color:"#FFA500 ", opacity:.75},    
    onEachFeature: function (feature, layer){
        layer.on({
...
var layer5 = L.geoJson(null, {
    ...
    onEachFeature: function (feature, layer){			
        layer.on({											
            click: highlightLayer5,					//click function handling specific to data type (one for point and one for polygon)	--> created in actions.js								
            dblclick: zoomToPoint								
        });
		layer5Search.push({	
            name: layer.feature.properties.NAME,
