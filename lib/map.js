...
var baseLayers = {
	...
	"Road Signs": Esri_transportation
};
...
var layer1icon = L.OpenFreightMarkers.icon({
        icon: 'bridge', markerColor: 'blue', layer:'layer1', title: 'Bridge/Intersection'}, IconPresets),
    layer2icon = L.OpenFreightMarkers.icon({
        icon: 'nhs', markerColor: 'purple', layer:'layer2', title: 'Road/Rail/Trail'}, IconPresets),
	layer3icon = L.OpenFreightMarkers.icon({
        icon: 'community', markerColor: 'teal', layer:'layer3', title: 'Area-wide Projects'}, IconPresets),
	layer4icon = L.OpenFreightMarkers.icon({
        icon: 'nhs', markerColor: 'red', layer:'layer4', title: 'Highway Safety Improvement Program'}, IconPresets);
...
var layer4pt = L.geoJson(null, {
    ...
    onEachFeature: function (feature, layer){			
        layer.on({											
            click: highlightLayer4,					//click function handling specific to data type (one for point and one for polygon)	--> created in actions.js								
            dblclick: zoomToPoint								
        });
    }
});  
$.getJSON("data/HSIP2017pt.json", function (data) {
	layer4pt.addData(data);
});

var layer4poly = L.geoJson(null, {
    style: {weight:3, color:"#d60004 ", opacity:.75},  
    onEachFeature: function (feature, layer){
        layer.on({
...
$.getJSON("data/HSIP2017poly.json", function (data) {
